    ...
    const CustomMobileLinks = ({ href, title, className = '', toggle }) => {
      ...
      return (
        <Link
          ...
          onClick={toggle} // ✅ Clicking the link will close the modal
        >
          {title}
          <span
            className={`h-[1px] inline-block bg-light dark:bg-dark absolute left-0 -bottom-0.5 
            group-hover:w-full group-active:w-full transition-[width] ease duration-300 
...
<span className={`bg-dark dark:bg-light block transition-all duration-300
  ...
  >
    ...
    <nav className="flex items-center justify-center flex-wrap">
      <motion.a href="/" whileHover={{ y: -2 }} whileTap={{ scale: 0.9 }} className="w-6 mx-3 sm:mx-1">
      ...
      <motion.a href="/" whileHover={{ y: -2 }} whileTap={{ scale: 0.9 }} className="w-6 ml-3">
        <DarkModeToggle />
      </motion.a>
    </nav>
  </motion.div>
